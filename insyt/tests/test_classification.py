...
class TestClassify(unittest.TestCase):
    ...
    def test_classifyno_attacks(self, mock_run_model, mock_database):
        # Setup
        ...
        mock_database_instance.fetch_sql.assert_called_once_with(
            "SELECT * FROM insyt WHERE classification IS NULL"
        )
        mock_run_model.assert_called_once_with("line", "context")
        mock_database_instance.update.assert_called_once_with(
            1, classification=CATEGORIES[0]
        )

    @patch("insyt.classification.Database", autospec=True)
    @patch("insyt.classification.run_model", return_value=1)
    def test_classifywith_attacks(self, mock_run_model, mock_database):
        # Setup
        mock_database_instance = mock_database.return_value
        mock_database_instance.fetch_sql.return_value = [
            (
